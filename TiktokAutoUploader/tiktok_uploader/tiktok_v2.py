...
# Local Code...
def upload_video(session_user, video, title, schedule_time=0, allow_comment=1, allow_duet=0, allow_stitch=0, visibility_type=0, brand_organic_type=0, branded_content_type=0, ai_label=0, proxy=None, products=None):
	try:
	...
	try:
		...
		print("[-] Failed to get 'project' from response. Raw response:")
		print(r.text)
		return False
	...
	# Add product anchors to the payload if products are provided.
	# Each item in `products` is a dict: {"product_id": str, "caption": str}
	# Mirrors the payload captured in mitmproxy: type=33, content.product_id,
	# content.keyword, content.add_from=2.
	if products:
		data["feature_common_info_list"][0]["anchors"] = [
			{
				"type": 33,
				"keyword": item.get("caption") or item.get("product_id", ""),
				"content": {
